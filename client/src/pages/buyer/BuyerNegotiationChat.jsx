...
const BuyerNegotiationChat = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Failed to load buyer negotiation chat:", {
          negotiationId: id,
          message: err?.message,
        });
  ...
  const handleAccept = async () => {
    if (!uiState.canBuyerRespondToLatestOffer) return;
    ...
    try {
      ...
      setStatusInfo(`Offer accepted at Rs. ${Number(uiState.latestOffer?.offerAmount || 0)}`);
      await refreshNegotiationState();
    } catch (err) {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      setStatusInfo("Offer rejected. The negotiation remains open.");
      await refreshNegotiationState();
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-5">
        ...
        <Link to="/buyer/negotiations" className="text-sm text-indigo-600 hover:underline">
          Back to My Negotiations
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        <div className="md:col-span-2 bg-white border border-gray-200 rounded-lg p-4">
          ...
          <p className="text-sm text-gray-500 mt-1">Seller: {negotiation.sellerId?.name || "Seller"}</p>
      ...
      {uiState.canBuyerRespondToLatestOffer && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-4">
          <p className="text-sm text-amber-800 mb-3">
            Latest seller offer: <span className="font-semibold">Rs. {uiState.latestOffer.offerAmount}</span>
          </p>
          <div className="flex gap-2">
            <button
              type="button"
              onClick={handleAccept}
              disabled={sending || uiState.areActionsDisabled}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <form onSubmit={handleSendOffer} className="bg-white border border-gray-200 rounded-lg p-4 space-y-3">
          <h2 className="text-sm font-semibold text-gray-700">Send Offer</h2>
          ...
          <input
            ...
            className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            disabled={sending || uiState.isOfferSendDisabled}
          />
          <button
            type="submit"
            disabled={sending || uiState.isOfferSendDisabled}
...
export default BuyerNegotiationChat;
