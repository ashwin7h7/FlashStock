  ...
  useEffect(() => {
    const fetchNegotiations = async () => {
      try {
        ...
        const sellerNegotiations = (listRes.negotiations || []).filter(
          (n) => String(n.sellerId?._id || n.sellerId) === String(user?._id || "")
        );

        const withLastMessage = await Promise.all(
          sellerNegotiations.map(async (n) => {
            try {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Seller Negotiations</h1>
        <Link
          to="/seller/products"
      ...
      {sortedItems.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-lg p-6 text-gray-500">
          No negotiation threads yet. Buyer offers will appear here.
        </div>
      ) : (
        <div className="space-y-3">
          {sortedItems.map((item) => {
            const displayStatus = getDisplayStatus(item);
            const buyerName = item.buyerId?.name || "Buyer";
            ...
            return (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-gray-500 mt-1">Buyer: {buyerName}</p>
                ...
                <div className="flex flex-col items-end gap-2 shrink-0">
                  ...
                  <Link
                    to={`/seller/negotiations/${item._id}`}
                    className="text-sm bg-indigo-600 text-white px-3 py-1.5 rounded hover:bg-indigo-700"
                  >
                    Open Chat
                  </Link>
