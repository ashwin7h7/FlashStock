...
const SellerPickups = () => {
  ...
  const confirmPickup = async (pickupId) => {
    ...
    try {
      ...
      if (data.success) {
        setPickups((prev) =>
          prev.map((p) => (p._id === pickupId ? { ...p, sellerConfirmed: true, status: data.pickup?.status || p.status } : p))
        );
      }
    } catch (err) {
      console.error(err);
          ...
          {pickups.map((p) => (
            <div key={p._id} className="bg-white rounded-lg shadow p-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
              <div>
                ...
                <p className="text-sm text-gray-500">Buyer: {p.buyerId?.name || p.buyerId}</p>
              ...
              {!p.sellerConfirmed && p.status === "pending" && (
                <button
